...
function whmch_sms_license_install_db(&$licenta) {
   global $wpdb;
	$sql = "CREATE TABLE IF NOT EXISTS whmcs_grinMedia_licence(ID int, licensekey VARCHAR(500), localkey VARCHAR(500))";
	$wpdb->query($sql);
		$empty = $wpdb->get_col($wpdb->prepare("SELECT * FROM whmcs_grinMedia_licence LIMIT 1" ) );
if ( empty($empty) ){
	$license_key = $licenta;
$sql2 = "INSERT INTO whmcs_grinMedia_licence (ID, licensekey, localkey) VALUES ('1', '" .$license_key. "', 'None')";
$wpdb->query($sql2);
}
	$license_key = $licenta;
    		$sql3 = "UPDATE whmcs_grinMedia_licence SET `licensekey`='$license_key' WHERE ID = 1";
$wpdb->query($sql3);
}
function whmcs_sms_licence_get_localkey() {
	global $wpdb;
	$sql = "SELECT localkey FROM whmcs_grinMedia_licence WHERE ID = 1 ";
...
function whmcs_sms_licence_get_licencekey(){
	global $wpdb;
	$sql = "SELECT licensekey FROM whmcs_grinMedia_licence WHERE ID = 1";	
    $active_rows = $wpdb->get_results($sql);
...
function sms_add_sms_style () {
	$sms_echo = '/wp-content/plugins/classipress-sms-payment-gateway/includes/sms_style.css';
...
function cp_author_info_sms() {
	...
	if ( is_plugin_active('classipress-sms-payment-gateway/sms.php')) { 
	$auth_ID =  $curauth->ID;
	get_currentuserinfo();
...
if ( is_user_logged_in() ) {
	...
	include_once( ABSPATH . '/wp-content/plugins/classipress-sms-payment-gateway/includes/select_function.php' );
    } 
    }
}
add_action( 'cp_author_info', 'cp_author_info_sms');
?>